...
# @File    : yolov9_trt_calibrator.py
# @Software: PyCharm

"""
    这是定义YOLOv9校准集数据加载器的脚本
"""
...
class YOLOv9CalibrationDataloader(CalibrationDataloader):

    def __init__(self,logger,input_shape,calibrator_image_dir,data_type='float32'):
        """
        这是YOLOv9模型INT8量化校准数据集加载器的初始化函数
        Args:
            logger: 日志类实例
            input_shape: 输入形状
        ...
        """
        self.logger = logger
        super(YOLOv9CalibrationDataloader,self).__init__(input_shape=input_shape,
                                                         calibrator_image_dir=calibrator_image_dir,
                                                         data_type=data_type)
    def preprocess_image(self, image):
        """
        这是YOLOv9对单张图像进行预处理的函数
        Args:
            image: 图像，opencv格式
        Returns:
        """
...
def yolov9_trt_calibrator(logger,input_shape,calibrator_image_dir,data_type,calibrator_table_path):
    """
    这是YOLOv9的TensorRT推理引擎INT8校准集加载器的注册函数
    Args:
        logger: 日志类实例
        input_shape: 模型输入尺寸
    ...
    Returns:
    """
    calibration_dataloader = YOLOv9CalibrationDataloader(logger,input_shape,calibrator_image_dir,data_type)
